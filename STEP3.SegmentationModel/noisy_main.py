...
from monai_trainer import AMDistributedSampler, run_noisy_training
...
parser.add_argument('--tumor_size', default='tiny', type=str, help='tiny, small, medium, large')
parser.add_argument('--organ_type', default='pancreas', type=str)
parser.add_argument('--organ_model', default='pancreas', type=str)
...
def main_worker(gpu, args):
    ...
    val_channel_names=['val_pancreas_dice', 'val_tumor_dice']
    ...
    train_img = train_img_healthy
    train_lbl = train_lbl_healthy
    train_name = train_name_healthy
    ...
    train_loader = data.DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=args.workers, sampler=None, pin_memory=True)


    val_ds = data.Dataset(data=data_dicts_val, transform=val_transform)
    val_loader = data.DataLoader(val_ds, batch_size=1, shuffle=False, num_workers=args.workers//2, sampler=None, pin_memory=True)
    ...
    accuracy = run_noisy_training(model=model,
