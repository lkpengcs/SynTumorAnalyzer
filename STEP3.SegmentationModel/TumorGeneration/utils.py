...
def get_fixed_geo(mask_scan, tumor_type, organ_type):
    ...
    tiny_radius, small_radius, medium_radius, large_radius = 6, 8, 10, 15
...
def synthesize_early_tumor(ct_volume, organ_mask, organ_type, tumor_size, vqgan, tester):
    ...
    with torch.no_grad():
        ...
        for bs in range(organ_mask_np.shape[0]):
            ...
            synthetic_tumor_type = tumor_size
...
def synthesize_medium_tumor(ct_volume, organ_mask, organ_type, vqgan, sampler, ddim_ts=50):
    ...
    # np.save('ct_volume.npy', ct_volume.detach().cpu().numpy())
    # np.save('organ_mask.npy', organ_mask.detach().cpu().numpy())
    # np.save('final_volume.npy', final_volume_.detach().cpu().numpy())
    # np.save('organ_tumor_mask.npy', organ_tumor_mask.detach().cpu().numpy())
    # exit()
    return final_volume_, organ_tumor_mask
